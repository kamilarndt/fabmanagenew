...
export function MagazynComplete() {
  ...
  return (
    <ConstructorContainer size="xl" padding="lg" className="w-full">
      <ConstructorSection
        title="Magazyn"
        subtitle="Zarządzanie stanami magazynowymi i bazą materiałów"
        spacing="lg"
      >
        {/* Header */}
        ...
        <ConstructorGrid cols={4} gap="md">
          <DashboardCard
            title={`${totalValue.toLocaleString('pl-PL')} PLN`}
            subtitle="Łączna wartość"
            icon={<Euro className="w-8 h-8 text-green-600 opacity-75" />}
          />

          <DashboardCard
            title={materials.length.toString()}
            subtitle="Pozycje magazynowe"
            icon={<Warehouse className="w-8 h-8 text-blue-600 opacity-75" />}
          />

          <DashboardCard
            title={lowStockCount.toString()}
            subtitle="Niski stan"
            icon={<AlertTriangle className="w-8 h-8 text-red-600 opacity-75" />}
          />

          <DashboardCard
            title="3 dni temu"
            subtitle="Ostatnie zamówienie"
            icon={<TrendingUp className="w-8 h-8 text-purple-600 opacity-75" />}
          />
        </ConstructorGrid>
        ...
        <DashboardCard
          ...
          subtitle="Znajdź i przefiltruj materiały"
          ...
          {filteredMaterials.map((material) => {
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full bg-muted rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all ${stockStatus === 'low'
                            ? 'bg-red-500'
                            : stockStatus === 'high'
                              ? 'bg-yellow-500'
                              : 'bg-green-500'
                ...
                {/* Actions */}
                <ConstructorFlex direction="row" gap="sm" className="pt-2">
                  <ActionButton action="secondary" size="sm" className="flex-1">
                    Edytuj
                  </ActionButton>
                  <ActionButton action="secondary" size="sm" className="flex-1">
                    Zamów
        ...
        {filteredMaterials.length === 0 && (
          <div className="text-center py-12">
            ...
            <ActionButton action="add" size="md" onClick={() => setShowMaterialsModal(true)}>
