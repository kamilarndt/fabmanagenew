...
import { BarChart3, TrendingUp, Users, Package, CheckCircle2, Clock, Calendar, AlertCircle, FolderOpen, AlarmClock, Mail, CheckSquare } from "lucide-react";
...
export function Dashboard({ onProjectClick }: DashboardProps = {}) {
  ...
  return (
    <ConstructorContainer size="xl" padding="lg" className="w-full">
      <ConstructorSection
        title="Dashboard"
        subtitle="Centrum dowodzenia - przegląd najważniejszych informacji i zadań wymagających uwagi"
        spacing="lg"
      >
        ...
        <ConstructorGrid cols={3} gap="lg">
          {kpiCards.map((card, index) => (
            <DashboardCard
              ...
              title={card.value}
              subtitle={card.title}
              icon={<card.icon className={`w-6 h-6 ${card.variant === 'warning' ? 'text-yellow-600' : 'text-muted-foreground'}`} />}
              className={card.variant === 'warning' ? 'border-yellow-200 bg-yellow-50/50' : ''}
            />
          ))}
        </ConstructorGrid>

        {/* Instrukcja 4.2 i 4.3: Lista Moje Zadania i Oś Czasu Projektów */}
        <ConstructorGrid cols={2} gap="lg">
          {/* Lista Moje Zadania */}
          <DashboardCard
            title="Moje Zadania"
            subtitle="Zadania wymagające uwagi"
              ...
              {myTasks.map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex-shrink-0">
                    <p className={`text-xs font-medium ${task.overdue && !task.completed ? 'text-red-600' : 'text-muted-foreground'
                      }`}>
                      {task.deadline}
                    </p>
                  </div>
          ...
          {/* Oś Czasu Projektów */}
          <DashboardCard
            title="Oś Czasu Projektów"
            subtitle="Harmonogram realizacji projektów"
