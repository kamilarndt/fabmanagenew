...
import TileCard from "../../tiles/components/TileCard";
import TileEditDrawer from "../../tiles/components/tile-edit-drawer";
import type { Tile } from "../../tiles/types/tiles.types";
...
export default function ProjectElements({
  ...
  const groups = useMemo(() => {
    ...
    if (!dict["Bez grupy"])
      dict["Bez grupy"] = { id: "ungrouped", name: "Bez grupy", tiles: [] };
  ...
  const handleTileSave = async (tileData: Omit<Tile, "id">) => {
    try {
      ...
      showToast(
        editingTile ? "Element zaktualizowany" : "Element utworzony",
        "success"
      );
      handleModalClose();
    } catch {
    ...
    <div>
      ...
      {projectTiles.length === 0 && (
        <Card style={{ marginBottom: 16, borderColor: "#ff6b35" }}>
          ...
          <div style={{ fontSize: 12, color: "#666" }}>
            <div>
              ID Projektu: <code>{project.id}</code>
            </div>
            <div>
              Nazwa Projektu: <code>{project.name}</code>
            </div>
            <div>
              Liczba wszystkich elementów: <code>{projectTiles.length}</code>
            </div>
            <div>
              Czy jest moduł 'projektowanie_techniczne':{" "}
              <code>
                {project.modules?.some((m) => m.name === "technical_design")
                  ? "TAK"
                  : "NIE"}
              </code>
            </div>
            <div>
              Dostępne moduły:{" "}
              <code>{project.modules?.join(", ") || "BRAK"}</code>
            </div>
          </div>
        </Card>
      )}
      ...
      >
        ...
        <Space>
          <Button
            ...
            aria-label="Stwórz nową grupę"
          >
            <i className="ri-add-circle-line" style={{ marginRight: 4 }}></i>
            Stwórz grupę
          </Button>
          {project.modules?.some((m) => m.name === "production") &&
            onPushToProduction && (
              <Button
                size="small"
                onClick={onPushToProduction}
                aria-label="Wyślij elementy do produkcji"
              >
                <i
        ...
        <Form layout="vertical" onFinish={handleQuickAdd}>
            ...
            <Col xs={24} md={10}>
              <Form.Item label="Nazwa elementu *" required>
                <Input
                  ...
                  value={qaName}
                  onChange={(e) => setQaName(e.currentTarget.value)}
                />
              </Form.Item>
            </Col>
            <Col xs={24} md={6}></Col>
          ...
          >
            ...
            <Button
              type="link"
              size="small"
              style={{ padding: 0, marginLeft: 4, verticalAlign: "baseline" }}
              onClick={handleTileCreate}
            >
              Otwórz edytor
            </Button>
          </div>
      ...
      <Tabs
        items={(() => {
          const items = [
            {
              ...
              children: (
                <div style={{ marginBottom: 16 }}>
                  <Row gutter={[16, 16]}>
                    {columns.map((column) => {
                      const columnTiles = projectTiles.filter(
                        (tile) => getColumnId(tile) === column.id
                      );
                      return (
                            ...
                            title={
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    backgroundColor:
                                      column.color === "bg-secondary"
                                        ? "#6c757d"
                                        : column.color === "bg-warning"
                                        ? "#ffc107"
          ...
          if (project.modules?.some((m) => m.name === "technical_design")) {
            items.push({
              key: "model3d",
              label: "Model 3D",
              children: (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const created = await createTileFromSelection(
                              project.id,
                              selectedObjectIds,
                            ...
                            if (created) {
                              showToast(
                                "Utworzono element z selekcji",
                                "success"
                              );
                              await useTilesStore.getState().refresh();
                              showNotification(
                                "Element utworzony",
                                `#${created.id} – kliknij Cofnij, aby usunąć`,
                                "info"
                              );
                            } else {
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          message.info(
                            "Funkcja przypisywania materiałów będzie dostępna wkrótce"
                          );
                        }}
                      >
      ...
      <div style={{ marginTop: 16 }}>
        ...
        >
          ...
          >
            <i className="ri-folder-add-line" style={{ marginRight: 4 }}></i>
            Nowa grupa
          </Button>
        </div>
        <GroupView groups={groups} onTileClick={onTileClick} />
      </div>
