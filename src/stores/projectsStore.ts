...
export type { Project, ProjectGroup, ProjectModule, ProjectWithStats };
...
interface ProjectsState {
  ...
  getProjectsByStatusAndModule: (
    status: Project["status"],
    module: ProjectModule
  ) => Project[];
  getProjectsByDeadlineRange: (
    days: number,
    status: Project["status"],
    module: ProjectModule
  ) => Project[];
  getProjectDataForGantt: (
    projectId: string
  ) => import("./calendarStore").GanttTask[];
}
  ...
  persist<ProjectsState, [], [], { projects: Project[] }>(
    (set, get) => ({
      ...
      initialize: async (force = false) => {
        ...
        // If we have cached projects but force is not set, check if we need to reload
        if (
          !force &&
          ...
          console.warn(
            "🔄 Projects store already initialized with",
            currentState.projects.length,
            "projects - checking if reload needed"
          );
          
          // Always reload from API to ensure data consistency
          console.warn("🔄 Forcing reload from API to sync with database...");
          // Continue to API call below instead of returning
        }

        // Prevent multiple simultaneous initializations
        if (!force && currentState.isLoading) {
          console.warn("🔄 Projects store is loading, skipping...");
          return;
        }
      ...
      syncWithClients: () => {
        setTimeout(() => {
          if (typeof window !== "undefined") {
            import("../stores/clientDataStore").then(
              ({ useClientDataStore }) => {
      ...
      clearCache: () => {
        console.warn("🧹 Clearing projects cache and forcing reload...");
        set({
          projects: [],
          projectsById: {},
          isInitialized: false,
        ...
        if (typeof window !== "undefined") {
          localStorage.removeItem("fabmanage-projects");
        }
      },
      ...
      remove: async (id: string) => {
        ...
        set((state) => ({
          ...
          projectsById: Object.fromEntries(
            Object.entries(state.projectsById).filter(([k]) => k !== id)
          ),
        }));
      ...
      getOverdueProjects: () => {
        ...
        return projects.filter(
          (p) => p.deadline && p.deadline < today && p.status !== "Zakończony"
        );
      },
      ...
      getLowProgressProjects: () => {
        const { projects } = get();
        return projects.filter(
          (p) => p.postep < 30 && p.status === "W realizacji"
        );
      },
      ...
      getProjectsByBudgetRange: (min: number, max: number) => {
        const { projects } = get();
        return projects.filter(
          (p) => p.budget && p.budget >= min && p.budget <= max
        );
      },
      ...
      getProjectsWithFiles: () => {
        const { projects } = get();
        return projects.filter(
          (p) => p.groups && p.groups.some((g) => g.files && g.files.length > 0)
        );
      },
      ...
      getProjectsWithoutFiles: () => {
        ...
        return projects.filter(
          (p) =>
            !p.groups || p.groups.every((g) => !g.files || g.files.length === 0)
        );
      },
      ...
      getProjectsByYear: (year: number) => {
        const { projects } = get();
        return projects.filter(
          (p) => new Date(p.data_utworzenia).getFullYear() === year
        );
      },
      ...
      getProjectsByBudgetCategory: () => {
        ...
        return {
          ...
          medium: projects.filter(
            (p) => (p.budget || 0) >= 50000 && (p.budget || 0) < 200000
          ),
      ...
      getProjectsByDuration: () => {
        ...
        return projects.sort((a, b) => {
          const aDuration = a.deadline
            ? new Date(a.deadline).getTime() -
              new Date(a.data_utworzenia).getTime()
            : 0;
          const bDuration = b.deadline
            ? new Date(b.deadline).getTime() -
              new Date(b.data_utworzenia).getTime()
            : 0;
      ...
      getProjectsByStatusAndModule: (
        ...
        const { projects } = get();
        return projects.filter(
          (p) => p.status === status && p.modules?.includes(module)
        );
      },
      ...
      getProjectDataForGantt: (projectId: string) => {
        ...
        const projStart =
          (project as any).start ||
          (project as any).data_utworzenia ||
          new Date();
        const ps = (projStart instanceof Date ? projStart : new Date(projStart))
          .toISOString()
          .slice(0, 10);
        tasks.push({
          id: project.id,
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state.projects && state.projects.length > 0) {
          state.projectsById = Object.fromEntries(
        ...
        try {
          if (config.enableRealtimeUpdates) {
            const unsubscribe = subscribeTable<Project>(
              "projects",
              (rows) => {
          ...
          console.warn(
            "Failed to setup realtime subscription for projects:",
            error
          );
        }
      },
