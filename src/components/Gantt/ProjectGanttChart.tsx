...
import { Card, Button, Select, Space, message, Modal, Form, Input, DatePicker, InputNumber, Tag } from 'antd'
import { PlusOutlined } from '@ant-design/icons'
// Frappe Gantt is loaded via CDN in index.html
import type { GanttTask, GanttTaskType, GanttTaskStatus, GanttTaskPriority, FrappeGanttTask } from '../../types/gantt.types'
...
const TASK_STATUSES: { value: GanttTaskStatus; label: string; color: string }[] = [
...
const TASK_PRIORITIES: { value: GanttTaskPriority; label: string; color: string }[] = [
...
export default function ProjectGanttChart({ projectId, tiles }: ProjectGanttChartProps) {
    ...
    const loadTasks = useCallback(async () => {
        ...
        setError(null)
        try {
            const response = await api.call<GanttTask[]>(`/api/gantt-tasks?project_id=${projectId}`, {
                method: 'GET',
                useSupabase: false
            })
            setTasks(response || [])
    ...
    const initializeGantt = useCallback(() => {
        ...
        if (!GanttClass) {
            ...
            message.error('Biblioteka harmonogramu nie została załadowana. Odśwież stronę.')
            return
        }
    ...
    const createTasksForTiles = async () => {
        ...
        try {
            ...
            for (const tile of tiles) {
                ...
                const taskTypes: GanttTaskType[] = ['projektowanie', 'wycinanie', 'produkcja']
                ...
                taskTypes.forEach((taskType, index) => {
                    ...
                    tasksToCreate.push({
                        ...
                        tile_id: tile.id,
                        name: `${tile.name} - ${TASK_TYPES.find(t => t.value === taskType)?.label}`,
                        task_type: taskType,
    ...
    const handleFormSubmit = async (values: any) => {
        try {
            ...
            setEditingTask(null)
            form.resetFields()
            message.success(editingTask ? 'Zadanie zostało zaktualizowane' : 'Zadanie zostało utworzone')
        } catch (error) {
            ...
            >
                ...
                >
                    ...
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Data rozpoczęcia jest wymagana' }]}
                    >
                    ...
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Data zakończenia jest wymagana' }]}
                    >
                        <DatePicker style={{ width: '100%' }} />
                    </Form.Item>
