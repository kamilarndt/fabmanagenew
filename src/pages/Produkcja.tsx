...
export default function Produkcja() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "queue" | "kanban" | "quality" | "maintenance"
  >("overview");
  const [lineFilter, setLineFilter] = useState<"Wszystkie" | string>(
    "Wszystkie"
  );
  ...
  const queueColumns: Column<Tile>[] = useMemo(
    () => [
      { key: "id", header: "ID", width: 80 },
      {
        key: "name",
        header: "Nazwa",
        render: (tile) => <div style={{ fontWeight: 600 }}>{tile.name}</div>,
      },
    ...
    <div>
      ...
      <Toolbar
          ...
          <Space>
            <Select
              ...
              options={[
                ...
                ...["Linia A", "Linia B", "Linia C", "Linia D"].map((l) => ({
                  value: l,
                  label: l,
                })),
              ]}
        ...
        right={
          <Space>
            <Button size="small">
              <i className="ri-download-line" style={{ marginRight: 6 }}></i>
              Eksport
            </Button>
          </Space>
        }
          ...
          <Col xs={12} lg={6} key={k.label}>
            <Card>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 20 }}>{k.value}</div>
                <Typography.Text type="secondary" style={{ display: "block" }}>
                  {k.label}
                </Typography.Text>
                <Typography.Text style={{ color: "var(--success-strong)" }}>
                  {k.extra}
                </Typography.Text>
              </div>
            </Card>
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              ...
              label: (
                <span>
                  <i className="ri-list-check" style={{ marginRight: 6 }}></i>
                  Kolejka
                </span>
              ),
            },
            ...
            },
            {
              ...
              label: (
                <span>
                  <i className="ri-tools-line" style={{ marginRight: 6 }}></i>
                  Utrzymanie ruchu
                </span>
              ),
            },
      ...
      {activeTab === "queue" && (
        <Card
          ...
          title={
            ...
            >
              <span>Kolejka montażowa ({assemblyTiles.length})</span>
              <Tag color="processing">Automatycznie z CNC</Tag>
            </span>
          }
        >
            ...
            <Empty
              description={
                <div>
                  ...
                  <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                    Elementy automatycznie pojawią się po zakończeniu cięcia CNC
                  </Typography.Text>
                </div>
              }
            />
      ...
      {activeTab === "kanban" && (
        <Card style={{ marginBottom: 12 }} title="Kanban">
          <KanbanBoardGeneric
            ...
            getColumnId={(t: any) => t.status as string}
            onItemClick={() => {}}
          />
        </Card>
      )}
              ...
              <div style={{ maxHeight: 360, overflowY: "auto" }}>
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {Array.from({ length: 16 }).map((_, i) => (
                    ...
                    >
                      {i % 2 ? "Start" : "Complete"} •{" "}
                      {orders[i % orders.length].name} • 08:{(10 + i) % 60}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <Card
              title={
                ...
                >
                  ...
                  <Button
                    size="small"
                    icon={(<i className="ri-refresh-line"></i>) as any}
                  />
                </span>
              }
            >
              <Row gutter={[8, 8]}>
                {Array.from({ length: 12 }).map((_, i) => (
                  <Col span={8} key={`test-${i}`}>
                    <Tag
                      color={i % 5 === 0 ? "error" : "success"}
        ...
        <Card
          title={
            ...
            >
              ...
              <Button size="small">
                <i className="ri-add-line" style={{ marginRight: 6 }}></i>Nowe
                zgłoszenie
              </Button>
            </span>
          }
              ...
              <tbody>
                {["CNC-01", "CNC-02", "MONTAŻ-01"].map((m, i) => (
                  <tr key={m}>
                    <td>{m}</td>
                    <td>
                      <Tag color={i === 0 ? "error" : "warning"}>
                        {i === 0 ? "Wysoki" : "Średni"}
                      </Tag>
                    </td>
                    <td>{i === 2 ? "Zamknięte" : "Otwarte"}</td>
                    <td>2025-09-1{i + 1}</td>
                    <td>
