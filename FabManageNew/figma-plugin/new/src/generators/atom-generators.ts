...
import { FABMANAGE_DESIGN_SYSTEM, getStatusColor, getTextColorForBackground } from '../config/design-system'

// Atom Generators - Create basic UI elements in Figma
export class AtomGenerators {

    // Generate Text component
    static async createText(properties: ComponentProperties): Promise<TextNode> {
        await figma.loadFontAsync({ family: properties.fontFamily || 'Inter', style: properties.fontWeight || 'Regular' })
        ...
        if (properties.fills && properties.fills.length > 0) {
            ...
            textNode.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
        }
    ...
    static async createButton(properties: ComponentProperties): Promise<FrameNode> {
        ...
        if (properties.fills && properties.fills.length > 0) {
            ...
            button.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.primary }]
        }
        ...
        if (properties.effects && properties.effects.length > 0) {
            button.effects = properties.effects as Effect[]
        }

        // Add text to button
        if (properties.text) {
            const textNode = await this.createText({
                text: properties.text,
                fontSize: properties.fontSize || 14,
                fontWeight: properties.fontWeight || 'Medium',
                fontFamily: properties.fontFamily || 'Inter',
                textAlign: 'CENTER',
                fills: [{ type: 'SOLID', color: getTextColorForBackground((button.fills[0] as SolidPaint)?.color || { r: 0, g: 0, b: 0 }) }]
            })
    ...
    static async createStatusBadge(properties: ComponentProperties): Promise<FrameNode> {
        ...
        let badgeColor = FABMANAGE_DESIGN_SYSTEM.colors.info
        if (properties.text) {
            badgeColor = getStatusColor(properties.text)
        }
    ...
    static async createProgressBar(properties: ComponentProperties, percentage: number = 0): Promise<FrameNode> {
        ...
        container.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.background.tertiary }]
    ...
    static async createAvatar(properties: ComponentProperties): Promise<EllipseNode> {
        ...
        if (properties.fills && properties.fills.length > 0) {
            ...
            avatar.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.background.tertiary }]
        }
    ...
    static async createIcon(properties: ComponentProperties): Promise<VectorNode> {
        ...
        if (properties.fills && properties.fills.length > 0) {
            ...
            icon.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.secondary }]
        }
    ...
    static async createTag(properties: ComponentProperties): Promise<FrameNode> {
        ...
        if (properties.fills && properties.fills.length > 0) {
            ...
            tag.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.primary }]
        }
    ...
    static async generateAtomVariants(component: AtomicComponent): Promise<FrameNode[]> {
        ...
        for (const variant of component.variants) {
            ...
            switch (component.id) {
                case 'text-title':
                case 'text-body':
                    const textNode = await this.createText(mergedProperties)
                ...
                case 'button-primary':
                    variantNode = await this.createButton(mergedProperties)
                    variantNode.name = `${component.name} - ${variant.name}`
                    break

                case 'badge-status':
                    variantNode = await this.createStatusBadge(mergedProperties)
                    variantNode.name = `${component.name} - ${variant.name}`
                    break

                case 'progress-bar':
                    variantNode = await this.createProgressBar(mergedProperties, 50)
                    variantNode.name = `${component.name} - ${variant.name}`
                    break

                case 'avatar':
                    const avatarNode = await this.createAvatar(mergedProperties)
                    variantNode = figma.createFrame()
                    variantNode.name = `${component.name} - ${variant.name}`
                ...
                case 'icon':
                    const iconNode = await this.createIcon(mergedProperties)
                    variantNode = figma.createFrame()
                    variantNode.name = `${component.name} - ${variant.name}`
                ...
                case 'tag':
                    variantNode = await this.createTag(mergedProperties)
                    variantNode.name = `${component.name} - ${variant.name}`
                    break
