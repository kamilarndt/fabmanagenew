...
import { OrganismGenerators } from './organism-generators'
...
export class DesignSystemGenerator {
  ...
  static async generateDesignSystem(): Promise<void> {
    try {
      ...
      // Generate atoms
      await this.generateAtoms(designSystemPage)

      // Generate molecules
      await this.generateMolecules(designSystemPage)

      // Generate organisms
      await this.generateOrganisms(designSystemPage)

      // Generate color palette
      await this.generateColorPalette(designSystemPage)

      // Generate typography scale
      await this.generateTypographyScale(designSystemPage)

      // Generate spacing scale
      await this.generateSpacingScale(designSystemPage)
  ...
  // Generate all atoms
  static async generateAtoms(page: PageNode): Promise<void> {
    ...
    atomsTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    ...
    for (const atom of atoms) {
      ...
      atomTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
      ...
      const baseComponent = await this.generateAtomComponent(atom)
      if (baseComponent) {
        baseComponent.x = 0
        baseComponent.y = 0
      ...
      // Generate variants
      const variants = await AtomGenerators.generateAtomVariants(atom)
  ...
  static async generateMolecules(page: PageNode): Promise<void> {
    ...
    moleculesTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    ...
    for (const molecule of molecules) {
      ...
      moleculeTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
      ...
      // Generate base component
      const baseComponent = await this.generateMoleculeComponent(molecule)
      if (baseComponent) {
        baseComponent.x = 0
        baseComponent.y = 0
        variantsContainer.appendChild(baseComponent)
      ...
      // Generate variants
      const variants = await MoleculeGenerators.generateMoleculeVariants(molecule)
      variants.forEach((variant, index) => {
        variant.x = (index + 1) * 120
        variant.y = 0
        variantsContainer.appendChild(variant)
  ...
  static async generateOrganisms(page: PageNode): Promise<void> {
    ...
    organismsTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    ...
    for (const organism of organisms) {
      ...
      organismTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
      organismGroup.appendChild(organismTitle)

      // Generate sample organism
      const sampleOrganism = await this.generateOrganismComponent(organism)
      if (sampleOrganism) {
        sampleOrganism.x = 0
        sampleOrganism.y = 0
  ...
  // Generate color palette
  static async generateColorPalette(page: PageNode): Promise<void> {
    ...
    colorTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    ...
    // Primary colors
    const primaryColors = [
      { name: 'Primary', color: FABMANAGE_DESIGN_SYSTEM.colors.primary },
      { name: 'Secondary', color: FABMANAGE_DESIGN_SYSTEM.colors.secondary },
      { name: 'Success', color: FABMANAGE_DESIGN_SYSTEM.colors.success },
      { name: 'Warning', color: FABMANAGE_DESIGN_SYSTEM.colors.warning },
      { name: 'Error', color: FABMANAGE_DESIGN_SYSTEM.colors.error },
      { name: 'Info', color: FABMANAGE_DESIGN_SYSTEM.colors.info }
    ]
    ...
    primaryColors.forEach((colorData, index) => {
      ...
      colorCard.effects = [FABMANAGE_DESIGN_SYSTEM.effects.shadows.sm as Effect]
  ...
  // Generate typography scale
  static async generateTypographyScale(page: PageNode): Promise<void> {
    ...
    typographyTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    typographySection.appendChild(typographyTitle)

    const fontSizes = Object.entries(FABMANAGE_DESIGN_SYSTEM.typography.sizes)
    ...
    fontSizes.forEach(([name, size]) => {
      ...
      textSample.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
  ...
  // Generate spacing scale
  static async generateSpacingScale(page: PageNode): Promise<void> {
    ...
    spacingTitle.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
    ...
    const spacingSizes = Object.entries(FABMANAGE_DESIGN_SYSTEM.spacing)
    ...
    spacingSizes.forEach(([name, size]) => {
      ...
      spacingCard.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.background.secondary }]
      spacingCard.strokes = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.border.primary }]
      ...
      spacingLabel.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.text.primary }]
      ...
      spacingVisual.fills = [{ type: 'SOLID', color: FABMANAGE_DESIGN_SYSTEM.colors.primary }]
  ...
  // Helper methods
  private static async generateAtomComponent(atom: any): Promise<FrameNode | null> {
    try {
      switch (atom.id) {
        case 'text-title':
        case 'text-body':
          const textNode = await AtomGenerators.createText(atom.properties)
        ...
        case 'button-primary':
          return await AtomGenerators.createButton(atom.properties)

        case 'badge-status':
          return await AtomGenerators.createStatusBadge(atom.properties)

        case 'progress-bar':
          return await AtomGenerators.createProgressBar(atom.properties, 50)

        case 'avatar':
          const avatarNode = await AtomGenerators.createAvatar(atom.properties)
        ...
        case 'icon':
          const iconNode = await AtomGenerators.createIcon(atom.properties)
        ...
        case 'tag':
          return await AtomGenerators.createTag(atom.properties)

        case 'space':
          return AtomGenerators.createSpace(atom.properties)
  ...
  private static async generateMoleculeComponent(molecule: any): Promise<FrameNode | null> {
    try {
      switch (molecule.id) {
        case 'card':
          return await MoleculeGenerators.createCard(molecule.properties)

        case 'form-input':
          return await MoleculeGenerators.createFormInput(molecule.properties)

        case 'space':
          return MoleculeGenerators.createSpace(molecule.properties)

        case 'project-thumbnail':
          return await MoleculeGenerators.createProjectThumbnail(molecule.properties)

        case 'project-content':
          return await MoleculeGenerators.createProjectContent(molecule.properties)

        case 'kanban-column':
          return await MoleculeGenerators.createKanbanColumn(molecule.properties)

        case 'tile-card':
          return await MoleculeGenerators.createTileCard(molecule.properties)
  ...
  private static async generateOrganismComponent(organism: any): Promise<FrameNode | null> {
    try {
      switch (organism.id) {
        case 'project-card':
          ...
          return await OrganismGenerators.createProjectCard(sampleProjectData)
        ...
        case 'project-elements':
          ...
          return await OrganismGenerators.createProjectElements(sampleProjectElementsData)
