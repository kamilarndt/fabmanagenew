import { FABMANAGE_DESIGN_SYSTEM } from './design-system'

// Ant Design Variables for Figma - dokładne odwzorowanie systemu z aplikacji
export class AntdDesignSystem {

    // Tworzenie zmiennych kolorów w Figmie
    static async createColorVariables(): Promise<void> {
        try {
    ...
    static async createTypographyVariables(): Promise<void> {
        try {
    ...
    static async createSpacingVariables(): Promise<void> {
        try {
    ...
    static async createBorderRadiusVariables(): Promise<void> {
        try {
            ...
            borderRadiusSm.setValueForMode(modeId, FABMANAGE_DESIGN_SYSTEM.effects.borderRadius.sm)

            const borderRadiusMd = figma.variables.createVariable("border-radius-md", borderRadiusCollection, "FLOAT")
            borderRadiusMd.setValueForMode(modeId, FABMANAGE_DESIGN_SYSTEM.effects.borderRadius.md)

            const borderRadiusLg = figma.variables.createVariable("border-radius-lg", borderRadiusCollection, "FLOAT")
            borderRadiusLg.setValueForMode(modeId, FABMANAGE_DESIGN_SYSTEM.effects.borderRadius.lg)

            const borderRadiusXl = figma.variables.createVariable("border-radius-xl", borderRadiusCollection, "FLOAT")
            borderRadiusXl.setValueForMode(modeId, FABMANAGE_DESIGN_SYSTEM.effects.borderRadius.xl)
    ...
    static async createAllVariables(): Promise<void> {
        await this.createColorVariables()
        await this.createTypographyVariables()
        await this.createSpacingVariables()
        await this.createBorderRadiusVariables()
    }

    // Pobieranie zmiennych z Figmy
    static getVariableByName(name: string): Variable | null {
    ...
    static getVariableValue(name: string): any {
    ...
    static createFillFromVariable(variableName: string): SolidPaint {
    ...
    static createStrokeFromVariable(variableName: string, weight: number = 1): SolidPaint {
        const color = this.getVariableValue(variableName)
        if (!color) {
            console.warn(`Variable ${variableName} not found, using fallback color`)
            return { type: 'SOLID', color: { r: 0, g: 0, b: 0 } }
    ...
    static getFontSizeFromVariable(variableName: string): number {
        const size = this.getVariableValue(variableName)
        return size || 14
    }

    // Pobieranie spacing z zmiennej
    static getSpacingFromVariable(variableName: string): number {
        const spacing = this.getVariableValue(variableName)
        return spacing || 8
    }

    // Pobieranie border radius z zmiennej
    static getBorderRadiusFromVariable(variableName: string): number {
