...
import { Card, Table, Button, Space, Typography, Row, Col, Statistic, Select, Input, Tag, Tooltip } from 'antd'
...
export default function MaterialsView({ tiles: propTiles }: MaterialsViewProps) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'Na stanie': return 'green'
            case 'Do zamówienia': return 'orange'
            case 'Zamówione': return 'blue'
            case 'Dostarczone': return 'green'
        ...
        },
        {
            ...
            render: (projectIds: string[]) => (
                <div>
                    ...
                    {projectIds.length > 2 && (
                        <Tooltip title={projectIds.slice(2).map(id => getProjectName(id)).join(', ')}>
                            <Tag style={{ fontSize: '0.75rem' }}>+{projectIds.length - 2}</Tag>
                        </Tooltip>
                    )}
                </div>
            )
        ...
        <div>
                ...
                <Row gutter={16} align="middle">
                    ...
                    <Col xs={24} sm={4}>
                        ...
                        >
                            ...
                            <Select.Option value="Materiał surowy">Materiał surowy</Select.Option>
            ...
            <Row gutter={16} style={{ marginBottom: 16 }}>
                ...
                <Col xs={24} sm={6}>
                    <Card>
                        <Statistic
                            title="Do zamówienia"
                            value={filteredMaterials.filter(m => m.status === 'Do zamówienia').length}
                ...
                <Col xs={24} sm={6}>
                    <Card>
                        <Statistic
                            title="Na stanie"
                            value={filteredMaterials.filter(m => m.status === 'Na stanie').length}
                            valueStyle={{ color: '#52c41a' }}
                        />
                    </Card>
                </Col>
            ...
            <Card>
                ...
                <Table
                    ...
                    scroll={{ x: 1000 }}
                    summary={(pageData) => {
                        const total = pageData.reduce((sum, item) => sum + item.totalCost, 0)
                        return (
                            <Table.Summary.Row>
