...
export const MaterialCardV3: React.FC<MaterialCardV3Props> = React.memo(({
  ...
  const stockStatus = React.useMemo(() => {
    ...
    if (ratio > (material.max_quantity || 100) / (material.min_quantity || 1)) return { 
      status: 'excess', 
      color: '#1890ff', 
  ...
  return (
    <Card
      ...
      style={{
        ...
        border: selected ? '2px solid var(--primary-main)' : '1px solid var(--border-main)',
        borderRadius: '8px',
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
          ...
          >
            {typeof thumbnail === 'string' ? thumbnail : thumbnail?.icon || '📦'}
          </div>
          ...
          <div style={{ marginBottom: '6px' }}>
            <div style={{ 
              ...
              <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                Stan: {material.quantity} / {material.min_quantity} {material.unit}
              </span>
              <Tag 
