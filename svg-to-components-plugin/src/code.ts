...
import { ReactComponentAnalysis, ParsedStructure } from './types/ast-types';
import { FigmaDesignTokens, EnhancedComponent } from './types/figma-types';
...
class EnhancedSVGConverter {
  ...
  private parseReactCode(code: string): ParsedStructure {
    try {
      ...
      // If no components were found, create a fallback component for testing
      if (components.length === 0) {
        console.log('⚠️ No components found, creating fallback component...');
        const fallbackComponent: ReactComponent = {
          name: 'TestComponent',
          props: { title: 'Test Component', description: 'This is a test component' },
          children: [],
          type: 'component',
          antdComponent: 'Card',
          figmaMapping: this.createFigmaMapping('TestComponent', '')
        };
  ...
  private extractReactComponents(code: string): ReactComponent[] {
    ...
    console.log('🔍 Extracting components from code...');
    console.log('Code length:', code.length);

    // Multiple regex patterns to catch different component formats
    const patterns = [
      // Function components: function ComponentName() { ... }
      /function\s+(\w+)\s*\([^)]*\)\s*\{[\s\S]*?return\s*\([\s\S]*?\)\s*;?\s*\}/g,
      // Arrow function components: const ComponentName = () => { ... }
      /const\s+(\w+)\s*=\s*\([^)]*\)\s*=>\s*\{[\s\S]*?return\s*\([\s\S]*?\)\s*;?\s*\}/g,
      // Arrow function components: const ComponentName = () => ( ... )
      /const\s+(\w+)\s*=\s*\([^)]*\)\s*=>\s*\([\s\S]*?\)/g,
      // Function components: const ComponentName = function() { ... }
      /const\s+(\w+)\s*=\s*function\s*\([^)]*\)\s*\{[\s\S]*?return\s*\([\s\S]*?\)\s*;?\s*\}/g,
      // Class components: class ComponentName extends React.Component { ... }
      /class\s+(\w+)\s+extends\s+React\.Component[\s\S]*?\{[\s\S]*?\}/g
    ];
