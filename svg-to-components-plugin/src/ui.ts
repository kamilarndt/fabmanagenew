...
class UIController {
  ...
  private createFigmaBtn!: HTMLButtonElement;
  ...
  private setupEventListeners(): void {
    ...
    this.createFigmaBtn = document.getElementById('create-figma-btn') as HTMLButtonElement;
    this.createFigmaBtn.addEventListener('click', () => this.createFigmaComponents());
    ...
    // React input change listener
    this.reactInput.addEventListener('input', () => {
  ...
  private createFigmaComponents(): void {
    const code = this.reactInput.value.trim();
    ...
    console.log('UI: Sending process-react message...');
    parent.postMessage({
      pluginMessage: {
        type: 'process-react',
        reactCode: code,
...
export default ProjectCard;`;
  ...
  private getSelectedComponents(): string[] {
    const selectedComponents: string[] = [];
    const checkboxes = document.querySelectorAll('#structure-list input[type="checkbox"]:checked');

    for (let i = 0; i < checkboxes.length; i++) {
      const checkbox = checkboxes[i] as HTMLInputElement;
      const elementIndex = parseInt(checkbox.dataset['index'] || '0');
      if (this.detectedElements[elementIndex]) {
        selectedComponents.push(this.detectedElements[elementIndex].name);
      }
    }

    // If no components are selected, return all component names
    if (selectedComponents.length === 0) {
      return this.detectedElements.map(el => el.name);
    }
  ...
  private updateButtonStates(): void {
    ...
    const hasReactCode = this.reactInput.value.trim().length > 0;

    this.analyzeReactBtn.disabled = this.isAnalyzing || this.isConverting;
    this.createFigmaBtn.disabled = this.isAnalyzing || this.isConverting || !hasReactCode;
